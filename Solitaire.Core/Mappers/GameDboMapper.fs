...
let mapGameOverviewDboToGame (game: GameOverviewDbo) : GameOverview =
...
let mapGameDboToGame (game: GameDbo) : Game =
    ...
    let stock =
        List.map mapCardDboToCard (List.ofSeq (game.Stock))

    let active =
        List.map mapCardDboToCard (List.ofSeq (game.Active))

    let wastepile =
        List.map mapCardDboToCard (List.ofSeq (game.Wastepile))
